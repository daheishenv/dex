<template>
  <div class="popup">
    ...
    <div class="mainTitle">{{ $t('apiTrade.toOfficialMedia') }}</div>
    <!-- 表单 -->
    <div class="form">
      <div class="item">
        <div class="subTitle">
          <span>{{ $t('apiTrade.mediaName') }}</span>
          <span class="color-red">*</span>
          <span class="tip"></span>
        </div>
        <div class="inputDiv">
          <input class="input" maxlength="255"
            :class="{'notNull': regError && !form.media.trim()}" type="text" v-model="form.media">
        </div>
      </div>
      <div class="item">
        <div class="subTitle">
          <span>{{ $t('apiTrade.contact') }}</span>
          <span class="color-red">*</span>
          <span class="tip">{{ $t('apiTrade.contactTip') }}</span>
        </div>
        <div class="inputDiv">
          <input class="input" maxlength="100"
            :class="{'notNull': regError && !form.contact.trim()}" type="text" v-model="form.contact">
        </div>
      </div>
      <div class="item">
        <div class="subTitle">
          <span>{{ $t('apiTrade.mediaLogo') }}</span>
          <!-- <span class="color-red">*</span> -->
          <span class="tip"></span>
        </div>
        <div class="inputDiv">
          <input type="file" ref="fileInput" class="fileInput" accept="image/*" @change="uploadPhoto($event)">
          <div class="fileInputDiv" @click="handleUploadImg">
            <span class="iconfont icon-huaban17 addImg"></span>
            <img width="100%" v-if="form.logo" :src="form.logo" alt="">
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    uploadPhoto(e) {
      ...
      reader.onload = (ev) => {
        ...
        const imgcode = ev.target.result;
        this.form.logo = imgcode;
      }
    },
    handleClose() {
    ...
    handleReg() {
      if (!this.form.media.trim() || !this.form.contact.trim()) {
        this.regError = true;
        return false;
      }
      this.regError = false;
