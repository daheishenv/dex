<template>
  <div class="morePage" :class="{'isVipDiv': vipInfo.level}" @click="handleCloseSelect">
    <div v-if="$appName === 'eosNewdex'">
         <!-- topShow -->
      <div class="more" v-if="isEosChain && tab === 2 && $store.state.app.vipInfo.level">
        <!-- 账户详情 -->
        <div class="accountVipDetail">
          <div class="charge" style="line-height: .33rem;">
            <div>
          ...
          <div class="saveCharge" style="line-height: .33rem;">
          ...
          <div class="mortgage stakeVip" v-else-if="accountInfo.vipMode === 1">
            <div class="openBtn" @click="handleToVip(2)">{{ $t('vip.updataShortMore') }}</div>
          </div>
        </div>
      </div>
    </div>
    <!-- 开通会员 & 升级会员 -->
    <div class="toVip" v-if="isEosChain && tab === 2">
      <!-- 会员过期 -->
      ...
      <!-- 升级会员 -->
      <div class="openVip" v-else-if="vipInfo.level === 0">
        <div>
    ...
    <div class="list">
      <!-- 会员中心 start -->
      <div class="list-item vipCenter" v-if="$appName === 'eosNewdex' && tab === 2">
        ...
        <!-- 领取box -->
        <div class="item" @click="handleReceiveBox" v-if="isEosChain && false">
          <div class="flex">
        ...
        <!-- 免费CPU -->
        <div class="item" @click="handleToCpu" v-if="isEosChain && false">
          <div>
      ...
      <!-- 客户服务 start -->
      <div class="list-item" v-if="tab === 3">
      ...
      <div class="list-item" v-if="tab === 3">
        ...
        <!-- 系统状态 -->
        <a class="item" href="/page/status">
          <div>
            <span class="iconfont icon-xitongzhuangtai icon info"></span>
          </div>
          <div class="name">
            <span style="color:#000">{{ $t('more.status') }}</span>
          </div>
          <div class="link">
            <span class="iconfont icon-huaban11"></span>
          </div>
    ...
    <mt-popup
      ...
      style="width: 90%;"
      popup-transition="popup-fade">
      <NodeList v-if="nodeListVisible" :nodeListVisible="nodeListVisible"  @listenNodeList="nodeListVisible = false"/>
    </mt-popup>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    */
    ...
    handleClose() {
      this.isRenew = false;
      this.receiveBoxVisible = false; // 领取box
    },
    // 抽奖
    handleToDraw() {
      this.$router.push({
