  ...
  <div class="linek" :class="{'isMove': isMove}">
    ...
    <div class="height">
      ...
      <div class="coinInfo" style="overflow:hidden">
        <div class="fl" :class="{'color-green': symbolData.change > 0,'color-red': symbolData.change < 0}">
          <div class="price">
            {{ Number(symbolData.price).toFixed(4) }}
        ...
        <div class="fr">
          ...
          <div>
            <span class="tip">
              <!-- <span class="iconfont icon-huaban106 switch"></span> -->
              <span v-if="$store.state.app.language !== 'ko'">24H</span><span>{{ $t('quotation.num') }}:</span>
            </span>
            <span>
              <span v-if="symbolData.amount < 10000000">{{ Number(symbolData.amount).toFixed(precision.coin) }}{{ symbol2 }}</span>
              <span v-else-if="symbolData.amount >= 10000000">{{ Number(symbolData.amount).toFixed(0) }}{{ symbol2 }}</span>
            </span>
          </div>
          <!-- 当前成交额 -->
    ...
    <div class="subTools">
      <div class="subTools-item" :class="{'fiexd boxShadow': channelRef === 'dapp' ? isScroll > 9.17 : isScroll > 10.17}">
        ...
        <!-- {{ $t('resources.data') }} -->
        <div @click="subActive = 2" :class="{'active': subActive === 2}">{{ $t('resources.mainData') }}</div>
...
import RamData from '@/views/resources/components/RamData'; // 数据
...
export default {
  ...
  methods: {
    ...
    handleLoadHeardWs() {
      ...
      // console.log(ticker, resources, 'ticker');
      if (ticker !== -1) {
