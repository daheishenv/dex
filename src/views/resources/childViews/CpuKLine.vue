<template>
  <div class="linek" :class="{'isMove': isMove}">
      ...
      <div class="tools" :class="{'fixed': channelRef === 'dapp',
        ...
        <div :class="{'pdleft': $store.state.app.channel !== 'meetone' || !isAndroid}">
          ...
          <span class="symbol">
            ...
            <div class="symbolDiv">
              <div>{{ `${symbol1} & ${symbol2} ${$t('resources.lease')}` }}</div>
    ...
    <div style="height: 1.27rem"></div>
      <div class="btnDiv">
        <div><button class="btn bgcolor-green" @click="handleToTrade('buy')">
          {{ $t('resources.rentCpu') }}
        </button></div>
        <div><button class="btn bgcolor-red" @click="handleToTrade('sell')">
          {{ $t('resources.lendCpu') }}
        </button></div>
      </div>
    </div>
...
// import NewDeal from '@/views/resources/components/NewDeal'; // 最新成交
import CpuData from '@/views/resources/components/CpuData'; // 数据
import CpuInfo from '@/views/resources/components/CpuInfo'; // 简介
