...
export default {
  ...
  getBars: function (symbol, resolution, from, to, callback) {
    ...
    const requestData = () => {
      ...
      // 请求并callback
      Io.cfwsKlineRam(params, (response, way) => {
  ...
  createdDataBtn(name, active, resolution) {
    ...
    button.onclick = function() {
      ...
      widget.chart().setResolution(resolution, function onReadyCallback(){
        ...
        klineConfig.intervalRam = resolution;
  ...
  */
  createdFillWinBtn() {
    ...
    button.title = _this.queryData.self.$t('nav.fullScreen');
    ...
    button.onclick = function() {
      ...
      button.innerHTML = '';
      button.title = _this.queryData.self.$t('nav.fullScreenOut');
